...
export default function LandingNavbar() {
  ...
  /* ── Visibility & opacity thresholds ──
     0–60px   : hidden (hero is visible)
     60–120px : fade in sliding down
     >120px   : fully visible
     on scroll back up same ranges in reverse
  */
  const FADE_START = 60;
  const FADE_END   = 120;

  const progress    = Math.min(Math.max((scrollY - FADE_START) / (FADE_END - FADE_START), 0), 1);
  const isVisible   = scrollY > FADE_START;
  ...
  return (
    <nav
      ...
      style={{
        ...
        borderBottom: `1px solid rgba(128,0,0,0.10)`,
    ...
    >
      ...
      <div style={{
        ...
        >
          ...
          <div>
            <div style={{ fontSize: 16, fontWeight: 800, color: '#111827', lineHeight: 1, letterSpacing: '-0.02em' }}>IskoMo</div>
            <div style={{ fontSize: 9.5, fontWeight: 700, color: MAROON, lineHeight: 1, marginTop: 2.5, letterSpacing: '0.07em', textTransform: 'uppercase' }}>PUP Main · OSFA</div>
          </div>
        </a>
          ...
          {[
            ...
            <li key={item.href}>
              <a
                ...
                style={{ display: 'block', padding: '8px 14px', borderRadius: 8, fontSize: 14, fontWeight: 600, color: '#374151', textDecoration: 'none', transition: 'color .2s, background .2s' }}
                onMouseEnter={e => { (e.currentTarget as HTMLElement).style.color = MAROON; (e.currentTarget as HTMLElement).style.background = 'rgba(128,0,0,0.06)'; }}
